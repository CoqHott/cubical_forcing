...
refine (J_seqs _ (fun p A x y => x y)
  (fun X _ => yft1 (A0 q α) q !
    (fun r β => J_seq _ (α ⋅ A0)
      (fun a _ => yft0 (a r β) r !)
      (X r (yft0 (A0 r (α ∘ β)) r !) (fun _ => x0 r (α ∘ β)) (αi ∘ β))
      (fun r (β : r ≤ q) => yft_funct β (A0 q α)) (A1 q α)
    )
  )
...
: El1 _ (eq1 (Arr0 A0 A1 B0 B1) (Arr1 A0 A1 B0 B1) f0 f1 g0 g1) (funext0 A0 A1 B0 B1 f0 f1 g0 g1 h0 h1).
Proof.
(* TODO : just plug things together *)
Admitted.